...
int cpupri_init(struct cpupri *cp)
{
	...
	// (&def_root_domain)->cpupri 값을 0으로 초기화
	...
	// for ((i) = -1; (i) = cpumask_next((i), (cpu_possible_mask)), (i) < nr_cpu_ids; )
		...
		// cp->cpu_to_pri: (&(&def_root_domain)->cpupri)->pri_to_cpu, i: 0, CPUPRI_INVALID: -1
		cp->cpu_to_pri[i] = CPUPRI_INVALID;
		// cp->cpu_to_pri[0]: (&(&def_root_domain)->cpupri)->pri_to_cpu[0]: -1
