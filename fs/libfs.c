 ...
 */
// ARM10C 20160521
// fs_type: &bd_type, "bdev:", &bdev_sops, NULL, 0x62646576
	...
	const struct dentry_operations *dops, unsigned long magic)
{
	...
	// name: "bdev:", strlen("bdev:"): 5
	// QSTR_INIT("bdev:", 5): { { { .len = 5 } }, .name = "bdev:" }
	struct qstr d_name = QSTR_INIT(name, strlen(name));
	// d_name.len: 5, d_name.name: "bdev:"

	// fs_type: &bd_type, MS_NOUSER: 0x80000000
	// sget(&bd_type, NULL, set_anon_super, 0x80000000, NULL): kmem_cache#25-oX (struct super_block)
	...
	// s->s_maxbytes: (kmem_cache#25-oX (struct super_block))->s_maxbytes, MAX_LFS_FILESIZE: 0x7ffffffffff
	s->s_maxbytes = MAX_LFS_FILESIZE;
	// s->s_maxbytes: (kmem_cache#25-oX (struct super_block))->s_maxbytes: 0x7ffffffffff

	// s->s_blocksize: (kmem_cache#25-oX (struct super_block))->s_blocksize, PAGE_SIZE: 0x1000
	s->s_blocksize = PAGE_SIZE;
	// s->s_blocksize: (kmem_cache#25-oX (struct super_block))->s_blocksize: 0x1000

	// s->s_blocksize_bits: (kmem_cache#25-oX (struct super_block))->s_blocksize_bits, PAGE_SHIFT: 12
	s->s_blocksize_bits = PAGE_SHIFT;
	// s->s_blocksize_bits: (kmem_cache#25-oX (struct super_block))->s_blocksize_bits: 12

	// s->s_magic: (kmem_cache#25-oX (struct super_block))->s_magic, magic: 0x62646576
	s->s_magic = magic;
	// s->s_magic: (kmem_cache#25-oX (struct super_block))->s_magic: 0x62646576

	// s->s_op: (kmem_cache#25-oX (struct super_block))->s_op, ops: &bdev_sops
	s->s_op = ops ? ops : &simple_super_operations;
	// s->s_op: (kmem_cache#25-oX (struct super_block))->s_op: &bdev_sops
	...
	root = new_inode(s);
	// root: kmem_cache#4-oX (struct inode)
	 ...
	 */
	// root->i_ino: (kmem_cache#4-oX (struct inode))->i_ino
	root->i_ino = 1;
	// root->i_ino: (kmem_cache#4-oX (struct inode))->i_ino: 1

	// root->i_mode: (kmem_cache#4-oX (struct inode))->i_mode,
	// S_IFDIR: 0040000, S_IRUSR: 00400, S_IRUSR: 00400
	root->i_mode = S_IFDIR | S_IRUSR | S_IWUSR;
	// root->i_mode: (kmem_cache#4-oX (struct inode))->i_mode: 0040400

	// root->i_atime: (kmem_cache#4-oX (struct inode))->i_atime,
	// root->i_mtime: (kmem_cache#4-oX (struct inode))->i_mtime,
	// root->i_ctime: (kmem_cache#4-oX (struct inode))->i_ctime, CURRENT_TIME: 현재시간값
	root->i_atime = root->i_mtime = root->i_ctime = CURRENT_TIME;
	// root->i_atime: (kmem_cache#4-oX (struct inode))->i_atime: CURRENT_TIME: 현재시간값
	// root->i_mtime: (kmem_cache#4-oX (struct inode))->i_mtime: CURRENT_TIME: 현재시간값
	// root->i_ctime: (kmem_cache#4-oX (struct inode))->i_ctime: CURRENT_TIME: 현재시간값

	// s: kmem_cache#25-oX (struct super_block), d_name.len: 5, d_name.name: "bdev:"
	// __d_alloc(kmem_cache#25-oX (struct super_block), &d_name): kmem_cache#5-oX (struct dentry)
	...
	// s->s_root: (kmem_cache#25-oX (struct super_block))->s_root, dentry: kmem_cache#5-oX (struct dentry)
	s->s_root = dentry;
	// s->s_root: (kmem_cache#25-oX (struct super_block))->s_root: kmem_cache#5-oX (struct dentry)

	// s->s_d_op: (kmem_cache#25-oX (struct super_block))->s_d_op, dops: NULL
	s->s_d_op = dops;
	// s->s_d_op: (kmem_cache#25-oX (struct super_block))->s_d_op: NULL

	// s->s_flags: (kmem_cache#25-oX (struct super_block))->s_flags: 0x80000000, MS_ACTIVE: 0x40000000
	s->s_flags |= MS_ACTIVE;
	// s->s_flags: (kmem_cache#25-oX (struct super_block))->s_flags: 0xc0000000

	// s->s_root: (kmem_cache#25-oX (struct super_block))->s_root: kmem_cache#5-oX (struct dentry)
	// dget((kmem_cache#25-oX (struct super_block))->s_root): kmem_cache#5-oX (struct dentry)
