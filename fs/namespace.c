...
// 2016/02/20 시작
	...
	// ida_get_new_above(&mnt_id_ida, 2, &(kmem_cache#2-oX)->mnt_id): 0
	// mnt_id_start: 3, &mnt->mnt_id: &(kmem_cache#2-oX)->mnt_id
	// ida_get_new_above(&mnt_id_ida, 3, &(kmem_cache#2-oX)->mnt_id): 0
	...
	if (!res)
		...
		// mnt_id_start: 3, mnt->mnt_id: (kmem_cache#2-oX (struct mount))->mnt_id: 3
 ...
 */
// ARM10C 20160521
// kmem_cache#2-oX (struct mount), 1
...
static struct mount *alloc_vfsmnt(const char *name)
{
	...
	if (mnt) {
		...
		// idr_layer_cache를 사용하여 struct idr_layer 의 메모리 kmem_cache#21-oX를 1 개를 할당 받음
		//
		// (&(&mnt_id_ida)->idr)->id_free 이 idr object new 4번을 가르킴
		// |
		// |-> ---------------------------------------------------------------------------------------------------------------------------
...
// 2016/03/19 시작
		...
		if (name) {
			...
			// mnt->mnt_devname: (kmem_cache#2-oX (struct mount))->mnt_devname, name: "bdev", GFP_KERNEL: 0xD0
			// kstrdup("bdev", GFP_KERNEL: 0xD0): kmem_cache#30-oX: "bdev"
			// mnt->mnt_devname: (kmem_cache#2-oX (struct mount))->mnt_devname, name: "proc", GFP_KERNEL: 0xD0
			// kstrdup("proc", GFP_KERNEL: 0xD0): kmem_cache#30-oX: "proc"
			...
			// mnt->mnt_devname: (kmem_cache#2-oX (struct mount))->mnt_devname: kmem_cache#30-oX: "bdev"
			// mnt->mnt_devname: (kmem_cache#2-oX (struct mount))->mnt_devname: kmem_cache#30-oX: "proc"
...
// 2016/04/16 시작
	...
	// name: "bdev", alloc_vfsmnt("bdev"): kmem_cache#2-oX (struct mount)
	// name: "proc", alloc_vfsmnt("proc"): kmem_cache#2-oX (struct mount)
...
// 2015/11/14 시작
	...
	// mount_fs(&bd_type, 0x400000, "bdev", NULL): kmem_cache#5-oX (struct dentry)
	// type: &proc_fs_type, flags: 0x400000, name: "proc", data: &init_pid_ns
	// mount_fs(&proc_fs_type, 0x400000, "proc", &init_pid_ns): kmem_cache#5-oX (struct dentry)
...
struct vfsmount *mntget(struct vfsmount *mnt)
{
	...
	if (mnt)
		// mnt: &(kmem_cache#2-oX (struct mount))->mnt
		// real_mount(&(kmem_cache#2-oX (struct mount))->mnt): kmem_cache#2-oX (struct mount)
...
// 2016/05/21 시작
	...
	if (!IS_ERR(new_ns)) {
		...
		// mnt->mnt_ns: (kmem_cache#2-oX (struct mount))->mnt_ns, new_ns: kmem_cache#30-oX (struct mnt_namespace)
		mnt->mnt_ns = new_ns;
		// mnt->mnt_ns: (kmem_cache#2-oX (struct mount))->mnt_ns: kmem_cache#30-oX (struct mnt_namespace)

		// new_ns->root: (kmem_cache#30-oX (struct mnt_namespace))->root, mnt: kmem_cache#2-oX (struct mount)
		new_ns->root = mnt;
		// new_ns->root: (kmem_cache#30-oX (struct mnt_namespace))->root: kmem_cache#2-oX (struct mount)
...
static void __init init_mount_tree(void)
{
	...
	// mnt: &(kmem_cache#2-oX (struct mount))->mnt
	// create_mnt_ns(&(kmem_cache#2-oX (struct mount))->mnt): kmem_cache#30-oX (struct mnt_namespace)
	ns = create_mnt_ns(mnt);
	...
	// ns: kmem_cache#30-oX (struct mnt_namespace), IS_ERR(kmem_cache#30-oX (struct mnt_namespace)): 0
	if (IS_ERR(ns))
		panic("Can't allocate initial namespace");

	// init_task.nsproxy: &init_nsproxy
	// init_task.nsproxy->mnt_ns: (&init_nsproxy)->mnt_ns, ns: kmem_cache#30-oX (struct mnt_namespace)
	...
	// mnt->mnt_root: (&(kmem_cache#2-oX (struct mount))->mnt)->mnt_root: kmem_cache#5-oX (struct dentry)
...
struct vfsmount *kern_mount_data(struct file_system_type *type, void *data)
{
	...
	// type: &proc_fs_type, MS_KERNMOUNT: 0x400000, type->name: (&proc_fs_type)->name: "proc", data: &init_pid_ns
	// vfs_kern_mount(&proc_fs_type, 0x400000, "proc", NULL): &(kmem_cache#2-oX (struct mount))->mnt
